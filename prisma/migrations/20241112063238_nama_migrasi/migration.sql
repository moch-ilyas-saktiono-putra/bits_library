...
CREATE TABLE `member` (
    ...
    `tanggal_bergabung` DATE NULL DEFAULT (curdate()),
...
CREATE TABLE `transaksi_pengembalian` (
    ...
    `nama_member` VARCHAR(255) NULL,
    `id_buku` INTEGER NULL,
    `judul_buku` VARCHAR(255) NULL,
    `tanggal_pengembalian` DATE NULL,
    `denda` DECIMAL(10, 2) NULL DEFAULT 0.00,

    UNIQUE INDEX `transaksi_pengembalian_id_transaksi_pinjam_key`(`id_transaksi_pinjam`),
    INDEX `id_transaksi_pinjam_idx`(`id_transaksi_pinjam`),
    INDEX `id_member_idx`(`id_member`),
    INDEX `id_buku_idx`(`id_buku`),
...
-- AddForeignKey
ALTER TABLE `transaksi_pengembalian` ADD CONSTRAINT `transaksi_pengembalian_ibfk_2` FOREIGN KEY (`id_member`) REFERENCES `member`(`id_member`) ON DELETE RESTRICT ON UPDATE RESTRICT;

-- AddForeignKey
ALTER TABLE `transaksi_pengembalian` ADD CONSTRAINT `transaksi_pengembalian_ibfk_3` FOREIGN KEY (`id_buku`) REFERENCES `buku`(`id_buku`) ON DELETE RESTRICT ON UPDATE RESTRICT;
